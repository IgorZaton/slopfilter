...
- **ONNX runtime setup required:** run `npm install` then `npm run build` (or `build:ml` + `build:icons`), then reload unpacked extension. Core content scripts are plain JavaScript loaded directly by Chrome. Build artifacts (`vendor/`, `offscreen.bundle.js`, generated icons) are gitignored — never commit them.
- **Store beta package:** `npm run package` → `dist/slopfilter-<version>.zip` (see `docs/CHROME_WEB_STORE_BETA.md`).
- **Icons:** source `icons/icon.svg`; run `npm run build:icons` for PNGs (gitignored).
